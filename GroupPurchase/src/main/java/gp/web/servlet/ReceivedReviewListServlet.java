...
public class ReceivedReviewListServlet extends HttpServlet {
	...
	public String getList(String studentNum)
	{
		...
		ArrayList<Review> reviewList = serv.getReviewListByEvaluateeNum(studentNum);
		if(reviewList.size() == 0) return "";
		for(int i=0; i< reviewList.size(); i++)
		{
			result.append("[{\"value\":\"" + reviewList.get(i).getReviewNum() + "\"},");
			result.append("{\"value\":\"" + reviewList.get(i).getRating() + "\"},");
			result.append("{\"value\":\"" + reviewList.get(i).getComment() + "\"},");
